...
class ext_update {
	...
	function main() {
		...
		if (!empty($update)) {
			...
			$tceData = array('tx_datafilter_filters' => array());
			foreach ($updateList as $uid => $newConfiguration) {
				$tceData['tx_datafilter_filters'][$uid] = array('configuration' => $newConfiguration);
			}
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid, title, configuration', 'tx_datafilter_filters', "configuration <> ''");
			// There are none
